...
This produces |bin/vision_core|, cross-compiled with the toolchain \term{Buildroot} builds as part of its own run, which must therefore already exist. A separate, native |make intel| target also exists, for exercising the pipeline on the host itself without the board.

\section{Usage}
\label{sec:fpgalix-vision-usage}

Beyond the \term{DE1-SoC} board itself, this requires an \term{OV7670} camera module and the custom camera adapter connecting it to the board's \term{GPIO\_0} header. The generic webcam source exists for the native |make intel| build, exercising the pipeline on the host itself; on the board, only the \term{OV7670} applies.
...
Once running, \term{SimpleUI} accepts filter-pipeline commands at its own prompt, and the \term{RTSP} stream is live at |rtsp://<board-ip>:8554/stream|.
