...
This chapter covers how to set up the host used for \prjname{FPGAlix}'s hardware and software development. It walks through installing the required software and where it lives on disk, then setting up the environment variables needed to reach it, and finally getting a working \gterm{usbblaster}{USB-Blaster}\footnote{\glsentrydesc{usbblaster}} and serial console access.
...
\prjname{FPGAlix} requires an \term{x86\_64} machine running \term{Ubuntu 22.04 LTS}, with \gterm{quartusprime}{Quartus Prime} (including \gterm{platformdesigner}{Platform Designer}) and an \term{ARM GNU} toolchain installed and working.
...
\begin{itemize}
    ...
    \item |ffmpeg|: provides \cmdpath{ffplay}, used on the host to view the \gterm{rtsp}{RTSP} video stream produced by \cmd{vision_core}. \term{VLC} was tried first and dropped due to compatibility problems.
\end{itemize}
