...
This produces |bin/fpgalix_camera.ko|, cross-compiled against the kernel source tree.
...
\textbf{Note:} All four changes are already committed to this project's \term{Linux} kernel fork (|linux-socfpga|), which arrives as a submodule. No manual patching is required.
...
The compiled module and the debug scripts used in the following steps are copied onto the board together, over |scp|, into a dedicated |camera_driver_2/| folder in the regular user's home directory:
...
Option 2, ``Deassert reset'', is selected. This releases |pclk_reset_controller|'s |sw_release| bit, a prerequisite for the camera to respond over \term{I\textsuperscript{2}C} at all.
