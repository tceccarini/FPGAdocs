...
\repopath{sw/camera_driver_2} is that driver: it captures frames through the \term{Altera} \term{mSGDMA} engine\footnote{See Chapter~\ref{chap:fpgalix-hw-overview}.} into a standard \extpath{/dev/videoN} device. Its development was assisted by \term{Claude Code}, an \term{AI} coding assistant, including the kernel-side analysis described in Section~\ref{sec:fpgalix-camera-short-frames}.
...
Select option 2, ``Deassert reset''. This releases \hw{pclk_reset_controller}'s \hw{sw_release} bit, a prerequisite for the camera to respond over \term{I\textsuperscript{2}C} at all.
