...
\begin{enumerate}
    ...
    \item \cmd{cv_bsp_generator.py} \cite{MACNICA-BOOT} (part of the \term{U-Boot} source tree) is run on the handoff folder, generating the board-specific header files (\repopath{pinmux_config.h}, \repopath{pll_config.h}, \repopath{sdram_config.h}, \repopath{iocsr_config.h}).
    ...
    \item \cmdpath{make} is invoked to build the bootloader image.
\end{enumerate}
...
\begin{warningbox}
    \term{Platform Designer}'s \term{IP} search path (Tools > Options > IP Catalog, ``IP Search Path'') is a \term{Quartus}-wide setting, not scoped to a single project. Every custom component used by this design (\hw{pclk_reset_controller}, \hw{ov7670_data_interface}, and the remainder found under \repopath{hw/my_vhdl/qsys_components/}, Chapter~\ref{chap:fpgalix-hw-overview}) resides there, and \term{Platform Designer} is able to resolve them only if that folder is listed. This setting should therefore be verified not only when configuring a new machine for the first time, but on every occasion where development switches between \prjname{FPGAlix} and another \term{Platform Designer} project on the same machine: each project maintains its own \repopath{hw/my_vhdl/qsys_components/}, and a search path left pointing at the other project's folder causes step 1 above to fail.
\end{warningbox}
...
The \cmdpath{--recurse-submodules} flag causes \term{Git} to additionally clone every submodule belonging to the project at the same time; these are located under \repopath{repos/} (including |u-boot-socfpga| \cite{SUB-UBOOT}, required in the following section, and |linux-socfpga| \cite{SUB-LINUX} and \cmdpath{buildroot} \cite{SUB-BUILDROOT}, required in Sections~\ref{sec:fpgalix-kernel} and \ref{sec:fpgalix-buildroot}). In its absence, the corresponding directories would be created but left empty.
...
Compilation of the hardware project causes \term{Quartus} to export the \term{HPS} configuration as \term{XML} files under \repopath{hw/quartus/hps_isw_handoff/soc_system_hps_0/}. This configuration is then converted into the \term{C} header files required by \term{U-Boot} by means of the following command:
...
The build produces \repopath{repos/u-boot-socfpga/u-boot-with-spl.sfp}, an image combining \term{SPL} and \term{U-Boot} in the \term{Altera} \term{SoCFPGA} format. This file is subsequently written, in raw form, to the boot partition of the \term{SD} card; it is also accessible via the symlink \repopath{sdcard/u-boot-with-spl.sfp}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{Multimedia support} $\rightarrow$ * (built-in, \hw{CONFIG_MEDIA_SUPPORT}). The option ``Filter media drivers'' should be left unchecked, as a consequence of which \hw{MEDIA_CAMERA_SUPPORT} and \ccode{VIDEO_DEV} are enabled automatically.
    \end{itemize}
    ...
    \begin{itemize}
        \item \texttt{Altera Soft IP I\textsuperscript{2}C} $\rightarrow$ * (built-in, \hw{CONFIG_I2C_ALTERA}). In the absence of this option, the \hw{altera_avalon_i2c} \term{Qsys} component (\texttt{compatible = "altr,softip-i2c-v1.0"}) remains invisible to \term{Linux}, preventing the camera from being configured over \term{I\textsuperscript{2}C} whenever the bus is routed through the \term{FPGA} rather than the \term{HPS}.
    \end{itemize}
    \item \texttt{Memory Management options}
    \begin{itemize}
        \item \texttt{Contiguous Memory Allocator} $\rightarrow$ * (\hw{CONFIG_CMA})
    \end{itemize}
    \item \texttt{Library routines}
    \begin{itemize}
        \item \texttt{DMA Contiguous Memory Allocator} $\rightarrow$ * (\hw{CONFIG_DMA_CMA}), \texttt{Size in Mega Bytes} $\rightarrow$ 16. This setting is recommended rather than strictly mandatory: the requirement of three buffers of 640$\times$480$\times$1 byte amounts to under 1 MB, so the 16 MB default provides an ample margin.
    \end{itemize}
\end{itemize}
...
\repopath{socfpga_cyclone5_fpgalix.dts} is not part of the upstream kernel; it was authored for this project and is already committed to this project's \term{Linux} kernel fork (|linux-socfpga|), which arrives as a submodule, matching the current hardware. If the \term{Platform Designer} hardware configuration has not changed, this file requires no further editing.
...
The compiled \term{DTB} is accessible via the symlink \repopath{sdcard/boot_partition/socfpga.dtb}, which points to \repopath{repos/linux-socfpga/arch/arm/boot/dts/socfpga_cyclone5_fpgalix.dtb}.

\subsection{Updating the Device Tree Source}
\label{subsec:fpgalix-dts-update}

This section only applies when the \term{HPS} subsystem has been modified in \term{Platform Designer}; otherwise \repopath{socfpga_cyclone5_fpgalix.dts} already matches the current hardware, and this section can be skipped.

\subsubsection{Using sopc2dts as a reference}

\repopath{sopc2dts} is \term{Altera}'s own tool for generating a device tree from a \repopath{.sopcinfo} file, tracked in this project as the \repopath{repos/sopc2dts} submodule (a fork of \extpath{altera-opensource/sopc2dts}). It cannot, however, be employed as a direct \term{DTS} source for the kernel: its output is a standalone file that does not include \repopath{socfpga_cyclone5.dtsi} and is therefore incompatible with the modern \term{Linux} \term{SoCFPGA} \term{DTS} infrastructure.

It remains useful, however, as a reference: its output reflects the current \term{Platform Designer} configuration, and is compared against \repopath{socfpga_cyclone5_fpgalix.dts} to identify which addresses, interrupt numbers and compatible strings changed, updating the latter accordingly.

The \repopath{sopc2dts} utility itself is not prebuilt: it is a plain \term{Java} tool (\cmdpath{javac}/\cmdpath{jar}, no \term{Quartus} or \term{Buildroot} toolchain involved); compile it from source once, the first time it is used:
...
In this project, \repopath{socfpga_cyclone5_fpgalix.dts} was authored from the \repopath{sopc2dts} reference output using \term{Claude Code}, an \term{AI} coding assistant, rather than written by hand. The reference \term{DTS} generated by \repopath{sopc2dts} (\extpath{/tmp/socfpga_ref.dts} above) was given as input, not the raw \repopath{soc_system.sopcinfo}: providing the latter directly led to significantly less accurate results, since it left the extraction of addresses, interrupt numbers and compatible strings to the \term{AI} assistant itself, rather than to \repopath{sopc2dts}. Manual authoring remains entirely possible: \repopath{socfpga_cyclone5_fpgalix.dts} is a plain text file, and the task consists solely of transcribing addresses, interrupt numbers and compatible strings from the reference output, for those who prefer to do so directly.
...
This last setting is not selected automatically: enabling ``Enable VFP extension support'' makes several floating-point strategies available (\term{VFPv3}, \term{NEON}, among others), and since the default chosen by \term{Buildroot} is not \term{NEON}, it must be selected explicitly. The \term{NEON}-accelerated \term{OpenCV} and \term{libjpeg-turbo} code of \repopath{sw/vision_core} depends on this setting.

\cmdpath{ifconfig} needs no package of its own: it is provided by \term{BusyBox} (\hw{CONFIG_IFCONFIG}), already enabled by default in this project's \term{BusyBox} configuration. \cmdpath{ip addr}/\cmdpath{ip route}, from \term{iproute2} above, are also available.
...
Do not use the \lstinline[style=inlineCmd]!-j$(nproc)! flag: \term{Buildroot} manages parallelism internally. The default settings already instruct \term{Buildroot} to employ all available \term{CPU} cores throughout the entire build (toolchain, packages, and root filesystem).
...
This same build also produces the toolchain used by the \cmdpath{make arm} target of \repopath{sw/vision_core}, at \repopath{repos/buildroot/output/host/bin/}; this \term{Buildroot} build must therefore complete before the \cmd{vision_core} program can be cross-compiled.

\section{Generating the FPGA bitstream (RBF)}
\label{sec:fpgalix-bitstream}

The \term{FPGA} fabric on the \term{DE1-SoC} may be configured through several paths: over \term{JTAG} from \term{Quartus}, over \term{JTAG} via a \term{JTAG} adapter tool, or by the \term{HPS} itself at boot, through the \term{FPGA Manager}. \prjname{FPGAlix} employs the last of these: \term{U-Boot} loads the bitstream from the \term{SD} card and programs the fabric through the \term{FPGA Manager}, prior to booting \term{Linux}.
...
The \term{SD} card requires three partitions: a \term{FAT32} boot partition (kernel, \term{DTB}, bitstream, boot script), an \term{ext4} root filesystem, and the raw \term{A2} partition for \term{U-Boot}. All files destined for the card are collected under \repopath{sdcard/}. A disk image file is constructed first and subsequently written to the physical \term{SD} card; the same procedure may be applied directly to the \term{SD} card by substituting the \term{SD} card device (e.g. \extpath{/dev/sdX}) for \repopath{sdcard/images/sdcard.img}, thereby omitting the image creation step and the final \cmdpath{dd}/\term{Etcher} write.

The final image may be written to the \term{SD} card using:

\begin{itemize}
    \item \term{Linux}: \cmdpath{dd} (see the last step below)
...
\term{SSH} access as \sysid{root} does not work, even though the serial console login does: \term{OpenSSH}'s default \sysid{PermitRootLogin} setting only allows \sysid{root} to log in with a key, not a password, and this project's \sysid{root} account only has a password. Create a new user, therefore, from the serial console:
...
The \cmdpath{mkdir -p /home} is necessary first: \term{Buildroot}'s base skeleton does not include a \extpath{/home} directory, and \term{BusyBox}'s \cmdpath{adduser} only creates the new user's home directory with a plain \ccode{mkdir()}, not \cmdpath{mkdir -p}. Without \extpath{/home} already in place, that \ccode{mkdir()} fails, silently, with nothing beyond a warning on the console; \cmdpath{adduser} still succeeds, but the new user is left with no home directory at all.
