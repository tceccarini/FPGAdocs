...
\begin{enumerate}
    ...
    \item Write the extracted image to the microSD card (at least 8 GB) with any disk-imaging tool, e.g. \cmdpath{dd} on \term{Linux} or \term{balenaEtcher} on Windows/Mac.
...
\begin{itemize}
    \item over \term{SSH}, once the IP is known, as \sysid{wheel}, then \cmdpath{su} for anything that needs root (loading \cmd{camera_driver_2}, accessing hardware registers directly):
...
\hw{ov7670} is mandatory on the board, the \ccode{webcam} source being only for the native \term{Intel} (\cmdpath{make intel}) build; \ccode{MJPEG}, left at its default by pressing Enter, is what this project recommends for full-color output here, since the \term{HPS} has no hardware video encoder.
...
The \prjname{FPGAlix} repository \cite{FPGALIX-REPO}, which is hosted on \term{GitHub}, is laid out as follows:

\begin{itemize}
    \item \repopath{hw/}: \term{FPGA} hardware project (\term{Quartus Prime} + \term{Platform Designer}).
    \begin{itemize}
        \item \repopath{camera_adapter/}: custom \term{PCB} bridging the \term{OV7670} to the \term{DE1-SoC}'s \term{GPIO\_0} header; the board layouts themselves are mirrored in the shared documentation folder (Chapter~\ref{chap:fpgalix-downloads}).
        \item \repopath{my_vhdl/}: custom \term{Platform Designer} \term{VHDL} components written for this project.
        \item \repopath{quartus/}: \term{Quartus} project, \repopath{soc_system.qsys}, top-level \term{VHDL}, pin assignments.
    \end{itemize}
    \item \repopath{sw/}: application software running on the board's embedded \term{Linux} (\term{C/C++}), one folder per component.
    \begin{itemize}
        \item \repopath{camera_driver_2/}: \term{V4L2} kernel driver for the \term{OV7670}.
        \item \repopath{hexip_daemon/}: displays the board's IP on the 7-segment displays.
        \item \repopath{vision_core/}: captures, filters and streams the camera feed over \term{RTSP}.
    \end{itemize}
    \item \repopath{repos/}: git submodules (\term{U-Boot}, \term{Linux} kernel, \term{Buildroot}, \term{GHRD}, \repopath{sopc2dts}, \term{Altera} \term{FPGA} top-level files).
