...
Readers who want the finished system running immediately, without going through every step described in this part, can start here instead. A prebuilt SD card image is available: it already contains the \gterm{bootloader}{bootloader}, \gterm{kernel}{kernel}, \gterm{rootfilesystem}{root filesystem} and \gterm{fpga}{FPGA}\footnote{\glsentrydesc{fpga}} bitstream, letting the \term{DE1-SoC} boot straight into a running system, without building anything from source.
...
\begin{itemize}
    ...
    \item The custom camera adapter (transmitter and receiver boards, connected by a ribbon cable), wiring the camera to the \term{DE1-SoC}'s \gterm{gpio0header}{\term{GPIO\_0} header}.
...
\begin{enumerate}
    ...
    \item Write the extracted image to the microSD card (at least 8 GB) with any disk-imaging tool, e.g. \gterm{dd}{\cmdpath{dd}} on \term{Linux} or \gterm{balenaetcher}{balenaEtcher}\footnote{\glsentrydesc{balenaetcher}} on Windows/Mac.
...
\begin{itemize}
    \item over \gterm{ssh}{SSH}, once the IP is known, as \sysid{wheel}, then \gterm{su}{\cmdpath{su}} for anything that needs root (loading \cmd{camera_driver_2}, accessing hardware registers directly):
...
\cmd{hexip_daemon} is already running once the board boots; no action needed. The prebuilt SD card image also already contains \cmd{camera_driver_2} and \cmd{vision_core}, built and placed in \sysid{wheel}'s own home directory (\extpath{/home/wheel/camera_driver_2/}, \extpath{/home/wheel/vision_core/}); no building or transferring files over \gterm{scp}{\cmdpath{scp}} is needed here. Building from source and deploying over \gterm{scp}{\cmdpath{scp}}, covered in full in later chapters, only matters after changing their source.
...
\hw{ov7670} is mandatory on the board, the \ccode{webcam} source being only for the native \term{Intel} (\gterm{make}{\cmdpath{make intel}}) build; \ccode{MJPEG}, left at its default by pressing Enter, is what this project recommends for full-color output here, since the \gterm{hps}{HPS} has no hardware video encoder.
