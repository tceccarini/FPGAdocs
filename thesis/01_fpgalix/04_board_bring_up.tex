...
Board bring-up turns the \term{DE1-SoC} from a board that merely powers on into the \term{Linux} execution platform on which the applications developed in the following chapters run.

This requires four components: the \gterm{uboot}{U-Boot} bootloader, the \term{Linux} kernel, the \gterm{buildroot}{Buildroot} root filesystem, and the \gterm{fpga}{FPGA} bitstream. \gterm{uboot}{U-Boot} initializes the hardware, configures the \gterm{fpga}{FPGA} fabric, and starts the kernel. The kernel provides the operating-system services and driver support required by the applications, including process scheduling, memory management, and the \gterm{v4l2}{V4L2} subsystem used by \cmd{camera_driver_2} and \cmd{vision_core}. The root filesystem provides the embedded user space and its libraries, including \gterm{opencv}{OpenCV}\footnote{\glsentrydesc{opencv}} and \gterm{gstreamer}{GStreamer}\footnote{\glsentrydesc{gstreamer}}. The bitstream configures the programmable logic with the custom hardware.

This chapter prepares these components and assembles them onto a bootable \term{SD} card, providing the baseline for the work that follows.
...
The \cmdpath{mkdir -p /home} is necessary first: \gterm{buildroot}{Buildroot}'s base skeleton does not include a \extpath{/home} directory, and if it is not already present, \gterm{busybox}{BusyBox}\footnote{\glsentrydesc{busybox}}'s \cmdpath{adduser} fails silently, leaving the new user without a home directory.
