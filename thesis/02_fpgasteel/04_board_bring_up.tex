...
Board bring-up turns the \term{DE1-SoC} from a board that merely powers on into the platform on which the bare-metal examples of the following chapters run.

This requires three components: the \gterm{uboot}{U-Boot} bootloader, the \gterm{fpga}{FPGA} bitstream, and a boot script. Unlike \prjname{FPGAlix}, no \term{Linux} kernel and no root filesystem are involved: \gterm{uboot}{U-Boot} initializes the hardware and configures the \gterm{fpga}{FPGA} fabric by loading the \gterm{fpga}{FPGA} bitstream. From there, the application is started in one of two ways:

\begin{itemize}
    \item \textbf{Standalone boot:} \gterm{uboot}{U-Boot} loads the compiled application (\repopath{.bin}) from the \term{SD} card and jumps to it.
    \item \textbf{\gterm{jtag}{JTAG} debugging:} \gterm{uboot}{U-Boot} stops once the fabric is programmed, and waits for the application (\repopath{.elf}) to be pushed from the host.
\end{itemize}

By choosing the boot script, the user selects which of the two ways \gterm{uboot}{U-Boot} takes, and \prjname{FPGAsteel} ships one for each.

This chapter prepares these components and assembles them onto a bootable \term{SD} card, providing the baseline for the examples that follow.
...
\term{ARM} toolchain (xPack Arm Embedded GCC) & Cross-compiles \gterm{uboot}{U-Boot} and the bare-metal example applications for the \term{DE1-SoC}'s \term{ARM} \term{Cortex-A9} \gterm{cpucore}{cores}. \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}
    ...
    \item Run \gterm{make}{\cmdpath{make}} to build the bootloader image.
\end{enumerate}
...
Find out which \gterm{loopdevice}{loop device} was assigned:
