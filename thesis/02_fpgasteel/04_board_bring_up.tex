...
This requires three components: the \gterm{uboot}{U-Boot} bootloader, the \gterm{fpga}{FPGA} bitstream, and a boot script. Unlike \prjname{FPGAlix}, no \term{Linux} kernel and no root filesystem are involved: \gterm{uboot}{U-Boot} initializes the hardware, configures the \gterm{fpga}{FPGA} fabric, and then loads the compiled application directly, either as a \repopath{.bin} read from the \term{SD} card at standalone boot or as an \repopath{.elf} pushed over \gterm{jtag}{JTAG} while debugging. The bitstream configures the programmable logic with the custom hardware. The boot script selects which of the two paths \gterm{uboot}{U-Boot} takes, and \prjname{FPGAsteel} ships one for each.
