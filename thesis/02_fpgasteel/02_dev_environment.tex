...
The development host is the \term{PC} on which \prjname{FPGAsteel}'s \gterm{fpga}{FPGA} project is compiled and the bare-metal applications that run on the \gterm{hps}{HPS} are cross-compiled; it is also the machine the \term{DE1-SoC} board stays connected to while it runs. This chapter gives the host the setup every later chapter assumes: the tools this part is built and debugged with, installed and reachable from the command line, and access to the two \term{USB} devices the board is reached through, the \gterm{usbblaster}{USB-Blaster II}\footnote{\glsentrydesc{usbblaster}} and the serial console.
