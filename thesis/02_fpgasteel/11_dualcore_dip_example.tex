...
All three take the same camera-to-\gterm{vga}{VGA} pipeline from \nameref{chap:fpgasteel-dualcore-forwarding} and replace the plain format conversion with a small computer-vision pipeline, \ccode{DualCoreDIP}: on every captured frame, it looks for one white ball against a black background and, if it finds one that is not touching the image border, draws its outline and a centroid cross on the \gterm{vga}{VGA} output. At a macroscopic level, the pipeline:
