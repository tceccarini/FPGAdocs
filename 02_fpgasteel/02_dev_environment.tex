...
\term{QUARTUS\_ROOTDIR} (or, as a fallback, \term{QSYS\_ROOTDIR}) is not just a convenience variable: \term{OpenOCD}'s own \term{JTAG} config script (\repopath{altera-usb-blaster2.cfg}, used from \term{Eclipse}'s \term{Debug Configuration}, Paragraph~\ref{subsec:fpgasteel-hello-debug-config}) reads it to locate \extpath{blaster_6810.hex}, the \term{USB-Blaster II} firmware file it needs to talk to the \term{JTAG} adapter.
