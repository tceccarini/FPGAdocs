...
The mechanism is exactly the one described in \nameref{chap:fpgasteel-dualcore-bringup} (Section~\ref{sec:fpgasteel-dualcore-operating-principles}): assert \term{CPU1}'s reset, write the two-word trampoline to address 0x0, write |cpu1startaddr| for good measure, clean the trampoline out of \term{L1} and \term{L2}, release the reset. |Core1::start()| (|Core1.cpp|) is that same sequence, with two differences worth noting, neither of which changes the underlying mechanism:
