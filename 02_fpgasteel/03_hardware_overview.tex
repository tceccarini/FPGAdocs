...
Very little carries over on the software side. Only one boot-chain submodule (|u-boot-socfpga|) is shared between the two repositories. \prjname{FPGAlix}'s own software, under its \repopath{sw/} (\cmd{camera_driver_2}, \cmd{hexip_daemon}, \cmd{vision_core}, embedded-\term{Linux} userspace code), has no counterpart here: \prjname{FPGAsteel}'s \repopath{sw/} is an unrelated, bare-metal example suite, documented starting from Board Bring-up.\footnote{See Chapter~\ref{chap:fpgasteel-board-bringup}.}
...
The hardware described here is common to every example under \repopath{sw/}, currently nine projects. They all target the same \repopath{soc_system.qsys} build and the same \repopath{FPGAsteel.sof}; what differs between them is the software, not the hardware.
...
Every \term{Platform Designer} instance on the capture path was renamed; the underlying custom \term{VHDL} is otherwise unchanged, with one exception, \hw{ov7670_data_interface}, covered below the table.
...
The underlying \term{IP} is unchanged in every case: the first two rows are this project's own custom cores (\hw{ov7670_data_interface}, \hw{pclk_reset_controller}); the remaining three are stock \term{Altera}/\term{Intel} \term{IP} (\hw{altera_avalon_mm_clock_crossing_bridge}, \hw{altera_avalon_dc_fifo}, \hw{altera_avalon_st_adapter}, in the same order).

\hw{ov7670_data_interface} gained a new register, \ccode{frame_size}. The hardware behavior is backward compatible with \prjname{FPGAlix}'s version. The \ccode{FRAME_SIZE} \term{VHDL} generic still exists, but it no longer fixes the frame size at synthesis time: it now only sets \ccode{frame_size}'s value on reset, as a default. From that point on, software can overwrite the register with a different byte count at run time, and the acquisition \term{FSM} uses whatever value the register currently holds. This is what lets the capture path switch between the \term{OV7670}'s 1-byte-per-pixel (\term{RAW}/processed \term{Bayer}) and 2-byte-per-pixel (\term{RGB565}/\term{RGB555}/\term{RGB444}/\term{YUV422}) output formats without recompiling the hardware.
...
\begin{itemize}
    ...
    \prjname{FPGAlix}'s \hw{sc_fifo_0}, a buffer \term{FIFO} placed there to mitigate frame loss when memory and \term{CPU} were under load, has no counterpart here. This is not simply because descriptors now live on-chip rather than in \term{DDR3}; it is because the whole re-arm path is faster and more deterministic than in \prjname{FPGAlix}: bare-metal software re-arms each descriptor from its interrupt handler with lower, more predictable latency than a \term{Linux} driver would, and the on-chip descriptor memory itself is read directly, at \term{SRAM} speed, without \term{DDR3}'s burst-oriented access and \term{MPFE} arbitration overhead. Together, these remove the latency \hw{sc_fifo_0} existed to absorb. How the prefetcher itself operates (descriptor layout, ping-pong buffering) is documented in full in Paragraph~\ref{subsec:fpgasteel-descriptors-prefetcher} and is not repeated here.
\end{itemize}

\section{VGA output path (new)}
\label{sec:fpgasteel-vga-output}

\prjname{FPGAsteel} adds a second, symmetric datapath in the \term{FPGA} fabric, \term{Avalon-MM}$\rightarrow$\term{ST} this time, that reads frames back out of \term{DDR3} and drives the \term{DE1-SoC}'s \term{VGA} connector directly, mirroring the capture path in reverse (software side: Paragraph~\ref{subsec:fpgasteel-simple-forwarding-vga-output}). None of the components below exist in \prjname{FPGAlix} \term{Platform Designer}'s system:

\begin{itemize}
    \item \textbf{\texttt{vga\_pll}} (\hw{altera_pll}): generates a fixed 25 \term{MHz} pixel clock from \hw{clk_0} (the board's 50 \term{MHz} oscillator, the same \hw{sys_clk} source everything else in the design uses).
    \item \textbf{\texttt{vga\_reset\_syncronizer}} (kind \hw{pll_lock_reset_sync}, a new custom component, source \repopath{hw/my_vhdl/qsys_components/pll_lock_reset_sync.vhd}, distinct from \hw{pclk_reset_controller}): releases the reset for the \hw{vga_clk} domain only once the system reset is released and \hw{vga_pll} reports locked.
    \item \textbf{\texttt{vga\_msgdma}} (\hw{altera_msgdma}, \term{MM}$\rightarrow$\term{ST} direction): the read-back counterpart of \hw{ov7670_video_msgdma}, equally prefetcher-enabled with its own on-chip descriptor \term{RAM}, \hw{vga_prefercher_descriptor_mem}.
    \item \textbf{\texttt{vga\_dc\_fifo}} crosses the stream from \hw{sys_clk} into the \hw{vga_clk} domain immediately after \hw{vga_msgdma.st_source}, while the stream is still kept at 64 bit wide.
    \item \textbf{\texttt{vga\_64\_to\_24\_repacker}} (kind \hw{avalon_st_64_to_24_repacker}, new custom \term{IP}, source \repopath{hw/my_vhdl/qsys_components/avalon_st_64_to_24_repacker.vhd}): named for the color depth it carries, \term{RGB888}, not for its output wire width. \hw{vga_controller}'s \term{Avalon-ST} sink is fixed by the University Program \term{IP} at 30 bit (\ccode{dataBitsPerSymbol}=10, \ccode{symbolsPerBeat}=3, \term{RGB101010}), and \term{Platform Designer}'s automatic width adapter cannot bridge an 8-bit-symbol stream to a 10-bit-symbol one, since the symbol width itself changes and not just the beat count. This component narrows the stream from 64 bit down to 24 bit, then zero-extends each 8-bit \term{RGB888} channel into its own 10-bit slot (2 zero low-order bits per channel), producing exactly the 30-bit \term{RGB101010} beat \hw{vga_controller} expects, one pixel per beat, entirely inside the \hw{vga_clk} domain, after the crossing above.
    \item \textbf{\texttt{vga\_controller}} (\hw{altera_up_avalon_video_vga_controller}, standard Altera University Program \term{IP}): drives the \term{DE1-SoC}'s \term{VGA} connector's sync and color signals at a fixed $640\times480$.
    \item A third dedicated \textbf{\term{FPGA}-to-\term{HPS} \term{SDRAM} port.} \prjname{FPGAlix} uses two: \hw{f2h_sdram0_data}, wired to a \term{JTAG} \term{Avalon} master (\hw{f2sdram_only_master}, kind \hw{altera_jtag_avalon_master}) for debug access, and \hw{f2h_sdram1_data} for its \term{mSGDMA}. \prjname{FPGAsteel} simply adds a third, \hw{f2h_sdram2_data}, for \hw{vga_msgdma}, for the same reason \prjname{FPGAlix} gives its own \term{mSGDMA} a dedicated port instead of routing through the \term{L3} interconnect (PART~I -- \prjname{FPGAlix}, Paragraph~\ref{sec:fpgalix-sdram-port}).
\end{itemize}

\section{System block diagram}
\label{sec:fpgasteel-system-block-diagram}

Figure~\ref{fig:fpgasteel-system-overview} shows \prjname{FPGAsteel}'s \repopath{soc_system.qsys}, redrawn from PART~I -- \prjname{FPGAlix}, Figure~\ref{fig:fpgalix-system-overview} (Section~\ref{sec:fpgalix-system-overview}). As described in Section~\ref{sec:fpgasteel-vga-output}, the \term{VGA} datapath, its hardware prefetcher, and its on-chip descriptor memory have been added; the camera capture path keeps its original layout, renamed per Paragraph~\ref{subsec:fpgasteel-renamed-instances} and re-labelled 64 bit per Paragraph~\ref{subsec:fpgasteel-msgdma-config}, with \hw{sc_fifo_0} removed and an on-chip descriptor \term{RAM} added for \hw{ov7670_video_msgdma}'s own prefetcher (Paragraph~\ref{subsec:fpgasteel-msgdma-config} explains why the former is no longer needed).
