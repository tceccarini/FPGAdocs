...
Same toolchain, \term{SD card} and \term{Eclipse}/\term{OpenOCD} debug setup as Hello World (Chapter~\ref{chap:fpgasteel-hello-world}),\footnote{TODO [nota 137]: footnote text was lost when the source was copy-pasted -- please supply it.} plus the \term{OV7670} camera adapter board on \term{GPIO\_0} as described in I\textsuperscript{2}C Example (Chapter~\ref{chap:fpgasteel-i2c-example}, Section~\ref{sec:fpgasteel-i2c-prerequisites}).\footnote{TODO [nota 138]: footnote text was lost when the source was copy-pasted -- please supply it.}

\begin{warningbox}
    \textbf{Hardware bug:} same camera \term{RESET}/\term{POWER-DOWN} caveat as I\textsuperscript{2}C Example.\footnote{TODO [nota 139]: footnote text was lost when the source was copy-pasted -- please supply it.}\footnote{TODO [nota 140]: footnote text was lost when the source was copy-pasted -- please supply it.}
\end{warningbox}

\section{About this project}
\label{sec:fpgasteel-cache-forwarding-about}

|cache_forwarding_example| (under |sw/cache_forwarding_example|) runs the exact same camera-to-\term{VGA} forwarding pipeline as |simple_forwarding_example|,\footnote{TODO [nota 141]: footnote text was lost when the source was copy-pasted -- please supply it.} over the same \term{FPGA} hardware, unchanged,\footnote{TODO [nota 142]: footnote text was lost when the source was copy-pasted -- please supply it.} but with the \term{CPU}'s data cache and the shared \term{L2} cache enabled. Enabling them means the frame buffers in \term{DDR} can be cached by the \term{CPU}, which is faster for the software side (|SimpleFilters::convert()|, in particular) but introduces a new problem absent from Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding}):\footnote{TODO [nota 143]: footnote text was lost when the source was copy-pasted -- please supply it.} the two \term{mSGDMA} cores that move frames in and out of \term{DDR} bypass the \term{ARM} cores' caches entirely, so the \term{CPU}'s cached view of a frame buffer can silently diverge from what is actually in \term{DDR}, the only copy the \term{DMA} cores ever see. Paragraph~\ref{subsec:fpgasteel-cache-mmu-architecture} explains why, and how this project resolves it.

Compared to Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding}),\footnote{TODO [nota 144]: footnote text was lost when the source was copy-pasted -- please supply it.} this project adds one step to startup: right after bringing up the interrupt controller, and before anything else runs, |System::enableCache()|\footnote{TODO [nota 145]: footnote text was lost when the source was copy-pasted -- please supply it.} enables the \term{L1} data cache, the \term{L2} cache, and the flat 1:1 section-based \term{MMU} mapping.

Everything else at startup, and in the main loop, is structurally the same as Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding}): |Camera| and |Vga| still run the same descriptor/prefetcher pipelines,\footnote{TODO [nota 146]: footnote text was lost when the source was copy-pasted -- please supply it.} and |SimpleFilters| still converts each captured frame to \term{RGB24} for display. The one loop-level difference is that frame buffers are now handled through |CacheAwareFrame|/|CacheAwareFramePool| (Paragraph~\ref{subsec:fpgasteel-cache-aware-frame}, Paragraph~\ref{subsec:fpgasteel-cache-aware-framepool}) instead of the plain |Frame|/|FramePool| from Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding}),\footnote{TODO [nota 147]: footnote text was lost when the source was copy-pasted -- please supply it.} so that the handoff between the \term{CPU} and each \term{DMA} master can carry an explicit cache flush or invalidate where one is needed.\footnote{TODO [nota 148]: footnote text was lost when the source was copy-pasted -- please supply it.}

The camera and \term{VGA} \term{ISR}s themselves, |Camera::onTransferComplete()| and |Vga::onTransferComplete()|, are unaltered: comparing this project's |Camera.cpp|/|Vga.cpp| against Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding})'s\footnote{TODO [nota 149]: footnote text was lost when the source was copy-pasted -- please supply it.} shows no logic difference at all.

\section{FPGA Hardware Involved}
\label{sec:fpgasteel-cache-forwarding-hardware}

Unchanged.\footnote{TODO [nota 150]: footnote text was lost when the source was copy-pasted -- please supply it.} Both \term{mSGDMA} cores, their descriptor memories, and every other \term{IP} core listed there are exactly the same hardware used by this project.
...
The Cyclone V SoC \term{FPGA} integrates a built-in dual-core \term{ARM} Cortex-A9 processor.\footnote{TODO [nota 151]: footnote text was lost when the source was copy-pasted -- please supply it.} Each core is provided with two separate 32 KB caches, one for data and one for instructions, plus its own \term{MMU} and \term{NEON}/\term{FPU} media processing engine.
...
\begin{figure}[htbp]
    ...
    \caption[Cortex-A9 MPU Subsystem Block Diagram]{Cortex-A9 MPU Subsystem Block Diagram, reproduced from the Cyclone V Hard Processor System Technical Reference Manual, Figure 10-2, p.\ 176.\footnotemark{} Reported here for reference; not original work.}
    \label{fig:fpgasteel-cortex-a9-mpu-subsystem}
\end{figure}
\footnotetext{TODO [nota 152]: footnote text was lost when the source was copy-pasted -- please supply it.}
...
The Cortex-A9 \term{L1} data cache cannot be turned on without the \term{MMU} also being on: cacheability and shareability are per-region attributes carried by the \term{MMU}'s translation tables, not a separate switch, so the cache literally has nowhere to read those attributes from until address translation is active. This project uses the simplest translation table that satisfies that requirement: a flat 1:1 mapping (every virtual address maps to the identical physical address), declared as three regions and handed to |alt_mmu_va_space_create()| (|HWLib|), which builds the actual hardware descriptors transparently, choosing whatever granularity each region's alignment and size allow. The project itself never deals with that choice:
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l l l l l p{4.5cm}@{}}
        ...
        \hline
    \end{tabular}
    \caption[MMU section mapping built by System::enableCache()]{\term{MMU} section mapping built by System::enableCache().\footnotemark{}}
    \label{tab:fpgasteel-mmu-section-mapping}
\end{table}
\footnotetext{TODO [nota 153]: footnote text was lost when the source was copy-pasted -- please supply it.}

The \term{DDR} region is the only one marked cacheable; the other two are left non-cacheable/device on purpose, since caching either register accesses or the descriptor memory the prefetcher polls\footnote{TODO [nota 154]: footnote text was lost when the source was copy-pasted -- please supply it.} would make software see stale values there too.
...
Unchanged.\footnote{TODO [nota 155]: footnote text was lost when the source was copy-pasted -- please supply it.} Everything there, the descriptor layout, the ping-pong loop between |desc0|/|desc1|, |OWNED_BY_HW|, and the prefetcher itself, applies here without modification.

\subsection{Camera capture internals}
\label{subsec:fpgasteel-cache-camera-capture}

Unchanged.\footnote{TODO [nota 156]: footnote text was lost when the source was copy-pasted -- please supply it.} |Camera::onTransferComplete()| runs the identical sequence; the only new step it now demands from its caller is the explicit |cacheInvalidate()| on the buffer it hands over,\footnote{TODO [nota 157]: footnote text was lost when the source was copy-pasted -- please supply it.} since the \term{ISR} itself has no reason to do that on the \term{DMA} core's behalf.

\subsection{VGA output internals}
\label{subsec:fpgasteel-cache-vga-output}

Unchanged.\footnote{TODO [nota 158]: footnote text was lost when the source was copy-pasted -- please supply it.} |Vga::onTransferComplete()| runs the identical sequence; the new step it now demands from its caller is the explicit |cacheFlush()| before a frame is pushed into |vgaPool|.\footnote{TODO [nota 159]: footnote text was lost when the source was copy-pasted -- please supply it.}

\section{Code structure}
\label{sec:fpgasteel-cache-code-structure}

Only the classes that differ from Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding})\footnote{TODO [nota 160]: footnote text was lost when the source was copy-pasted -- please supply it.} are documented in full here; everything else is covered by reference. Paragraphs~\ref{subsec:fpgasteel-cache-system-class}--\ref{subsec:fpgasteel-cache-aware-framepool} are entirely new in this project.

\subsection{Frame class}
\label{subsec:fpgasteel-cache-frame-class}

Unchanged,\footnote{TODO [nota 161]: footnote text was lost when the source was copy-pasted -- please supply it.} with one access-level change: |m_buffer| moved from private to protected, so that |CacheAwareFrame|\footnote{TODO [nota 162]: footnote text was lost when the source was copy-pasted -- please supply it.} can reach the raw pointer directly instead of going through |getBuffer()|. No behavioral change.

\subsection{FramePool class}
\label{subsec:fpgasteel-cache-framepool}

Unchanged,\footnote{TODO [nota 163]: footnote text was lost when the source was copy-pasted -- please supply it.} with two internal changes needed to support |CacheAwareFramePool|:\footnote{TODO [nota 164]: footnote text was lost when the source was copy-pasted -- please supply it.} |MAX_SLOTS| and the |m_frameStorage| bookkeeping array moved from private to protected, and |m_queueCount| changed from a plain |volatile unsigned| to |std::atomic<unsigned>|. Neither changes the public producer/consumer \term{API} (|borrow()|/|push()|/|pop()|/|giveBack()|) or its behavior.

\subsection{PrefetcherExtendedDescriptor class}
\label{subsec:fpgasteel-cache-prefetcher-extended-descriptor}

Unchanged.\footnote{TODO [nota 165]: footnote text was lost when the source was copy-pasted -- please supply it.}

\subsection{MsgdmaPrefetcherMemory class}
\label{subsec:fpgasteel-cache-msgdma-prefetcher-memory}

Unchanged.\footnote{TODO [nota 166]: footnote text was lost when the source was copy-pasted -- please supply it.} Descriptor memory is left non-cacheable by the \term{MMU} mapping (Subparagraph~\ref{subsubsec:fpgasteel-enabling-mmu}), so this class needs no cache maintenance of its own.

\subsection{PrefetchedMSGDMA class}
\label{subsec:fpgasteel-cache-prefetched-msgdma}

Unchanged.\footnote{TODO [nota 167]: footnote text was lost when the source was copy-pasted -- please supply it.}

\subsection{Camera class}
\label{subsec:fpgasteel-cache-camera-class}

Unchanged.\footnote{TODO [nota 168]: footnote text was lost when the source was copy-pasted -- please supply it.} |onTransferComplete()| is functionally identical to Simple Forwarding Example (Section~\ref{sec:fpgasteel-cache-forwarding-about} above).

\subsection{Vga class}
\label{subsec:fpgasteel-cache-vga-class}

Unchanged.\footnote{TODO [nota 169]: footnote text was lost when the source was copy-pasted -- please supply it.} |onTransferComplete()| is functionally identical to Simple Forwarding Example.

\subsection{SimpleFilters class}
\label{subsec:fpgasteel-cache-simple-filters}

Unchanged.\footnote{TODO [nota 170]: footnote text was lost when the source was copy-pasted -- please supply it.}

\subsection{HexDisplay class}
\label{subsec:fpgasteel-cache-hex-display}

Unchanged.\footnote{TODO [nota 171]: footnote text was lost when the source was copy-pasted -- please supply it.}

\subsection{System class}
\label{subsec:fpgasteel-cache-system-class}

|System| is the same singleton used in Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding})\footnote{TODO [nota 172]: footnote text was lost when the source was copy-pasted -- please supply it.} (|init()|, |micros()|, |millis()|, |delay()|, |delayMicroseconds()|, |deInit()|, all identical), plus one new method this project actually relies on: |enableCache()|. Source: |src/System.{h,cpp}|.
...
\begin{itemize}
    ...
    \item |CacheAwareFrame(buffer, bufferSize, width, height, colorScheme)| (overloaded constructor, form 2): wraps an existing buffer, exactly like |Frame|'s equivalent constructor;\footnote{TODO [nota 174]: footnote text was lost when the source was copy-pasted -- please supply it.} the caller is responsible for making sure that buffer is itself cache-line aligned.
...
|CacheAwareFramePool| extends |FramePool|\footnote{TODO [nota 175]: footnote text was lost when the source was copy-pasted -- please supply it.} with the same two operations, applied automatically at the point where a frame changes hands between producer and consumer, so that calling code cannot forget to call them. Source: |src/CacheAwareFramePool.{h,cpp}|.
...
\begin{itemize}
    ...
    \item |push(frame, flushCache)|: if |flushCache| is true, calls |frame->cacheFlush()| first, then forwards to |FramePool::push()|.\footnote{TODO [nota 176]: footnote text was lost when the source was copy-pasted -- please supply it.}
    \item |pop(invalidateCache)|: forwards to |FramePool::pop()|;\footnote{TODO [nota 177]: footnote text was lost when the source was copy-pasted -- please supply it.} if |invalidateCache| is true, calls |cacheInvalidate()| on the returned frame before returning it.
\end{itemize}
...
Neither |borrow()| nor |giveBack()| needs a cache operation of its own: both only touch bookkeeping state inside |FramePool|,\footnote{TODO [nota 178]: footnote text was lost when the source was copy-pasted -- please supply it.} never the pixel buffer itself.

\section{Debug tooling}
\label{sec:fpgasteel-cache-forwarding-debug-tooling}

|debugTools/view_frame.py|, same tool as in Simple Forwarding Example (Chapter~\ref{chap:fpgasteel-simple-forwarding}),\footnote{TODO [nota 179]: footnote text was lost when the source was copy-pasted -- please supply it.} works unchanged here. One caveat specific to this project: a raw memory dump (e.g.\ via \term{GDB} |dump memory|) reads \term{DDR} directly, not through the \term{CPU}'s caches, so a buffer whose cache lines have not been flushed yet may still show stale data in the dump. When inspecting a \term{CPU}-written buffer with this tool, dump it after the corresponding |cacheFlush()|,\footnote{TODO [nota 180]: footnote text was lost when the source was copy-pasted -- please supply it.} i.e.\ after |push(frame, true)| has run.

\section{References}
\label{sec:fpgasteel-cache-forwarding-references}

\begin{itemize}
    \item \textbf{[AVALON-SPEC]} Defines the \term{Avalon-ST} and \term{Avalon-MM} conventions used throughout Section~\ref{sec:fpgasteel-cache-forwarding-hardware}.
    \item \textbf{[EP-IPUG]} \term{mSGDMA} extended descriptor format and prefetcher operation, behind |PrefetcherExtendedDescriptor| and |PrefetchedMSGDMA| (used in Paragraph~\ref{subsec:fpgasteel-cache-prefetcher-extended-descriptor}, used in Paragraph~\ref{subsec:fpgasteel-cache-prefetched-msgdma}).
    \item \textbf{[OV7670-DS]} Register tables behind |Camera::configureSensor()| (used in Paragraph~\ref{subsec:fpgasteel-cache-camera-class}) and the pixel formats behind |SimpleFilters| (used in Paragraph~\ref{subsec:fpgasteel-cache-simple-filters}).
    \item \textbf{[CV-TRM]} Chapter 10, ``MPU Subsystem'': Figure 10-2 (Cortex-A9 MPU Subsystem Block Diagram, p.\ 176), cited in Paragraph~\ref{subsec:fpgasteel-cache-mmu-architecture}.
    \item \textbf{HWLib headers} (|alt_cache.h|, |alt_mmu.h|), from |intel-socfpga-hwlib|. Cache/\term{MMU} function reference behind |System::enableCache()| (used in Paragraph~\ref{subsec:fpgasteel-cache-system-class}) and |CacheAwareFrame|/|CacheAwareFramePool| (used in Paragraph~\ref{subsec:fpgasteel-cache-aware-frame}, used in Paragraph~\ref{subsec:fpgasteel-cache-aware-framepool}).
    \item \textbf{Quartus 22.1 HAL headers} (|altera_msgdma_csr_regs.h|, |altera_msgdma_prefetcher_regs.h|, |altera_msgdma_descriptor_regs.h|, |altera_msgdma_response_regs.h|). Register/bit definitions behind |PrefetchedMSGDMA| and |PrefetcherExtendedDescriptor|.
\end{itemize}
