...
Since \term{SoC EDS} v19.1 Std / v19.3 Pro, the bootloader (\term{U-Boot}) build flow no longer relies on \cmdpath{bsp-editor} to configure and generate \term{U-Boot}. The current flow is:
...
\textbf{Note:} in other projects this flow normally starts by cloning the \term{U-Boot} source from \term{GitHub}. In \prjname{FPGAsteel} this step is not needed: \repopath{repos/u-boot-socfpga} \cite{SUB-UBOOT} is already tracked as a \term{git} submodule (see \repopath{.gitmodules}), so the source is already in place after \cmdpath{git submodule update --init}.
...
\cmdpath{--recurse-submodules} makes \term{git} also clone every submodule listed in \repopath{.gitmodules} right away --- these live under \repopath{repos/} (including |u-boot-socfpga|, needed in the next section). Without it, those folders would be created but left empty.
...
\midrule
\repopath{multiboot.script} & Standalone boot. Shows a menu of the built example apps, loads the \term{FPGA} bitstream, loads the chosen app's \repopath{.bin} from the boot partition, and jumps to it with \cmdpath{go}. This is the ``Standalone: .bin'' path in Figure~\ref{fig:fpgasteel-boot-chain}. \\
