...
\textit{``Simple Forwarding Example''} (under \repopath{sw/simple_forwarding_example}) captures continuous video from the \term{OV7670} and forwards it to \term{VGA} output. At startup it:
...
\begin{itemize}
    \item converts each captured frame from \term{RGB565} to \term{RGB24} (\ccode{SimpleFilters});
...
\begin{enumerate}
    \item \textbf{\hw{ov7670_video_interface}} (|ov7670_data_interface|, custom \term{IP} inherited unchanged from \prjname{FPGAlix} and renamed here, see Paragraph~\ref{subsec:fpgasteel-renamed-instances}): reads the \term{OV7670}'s parallel pixel bus (\term{HREF}/\term{VSYNC}/8-bit data) and repackages it into a standard \term{Avalon-ST} byte stream, each frame as one packet, with \term{SOP} on the first byte and \term{EOP} on the last.\footnote{\term{SOP} and \term{EOP} stand for Start of Packet and End of Packet, respectively.}
    \item \textbf{\hw{ov7670_st_adapter}} (standard \term{Avalon-ST} width adapter \term{IP}): widens that stream from 8 bit to 64 bit, the width the \term{mSGDMA} below expects.
    \item \textbf{\hw{ov_7670_video_dc_fifo}} (dual-clock \term{FIFO}): crosses the stream from the camera's own pixel clock (\hw{ov7670_pclk}) into the \term{HPS}-facing clock domain; the two clocks are unrelated, so this crossing is mandatory, not just buffering.
    \item \textbf{\hw{ov7670_video_msgdma}}: an \term{mSGDMA} core (\term{ST}$\rightarrow$\term{MM} direction) with its prefetcher enabled. It writes each incoming packet into whichever \term{DDR} buffer the current descriptor points to; the descriptors themselves live in \hw{ov7670_prefercher_descriptor_mem}, a small on-chip \term{RAM}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{\hw{vga_msgdma}}: an \term{mSGDMA} core (\term{MM}$\rightarrow$\term{ST} direction) with its prefetcher enabled. It reads a frame from whichever \term{DDR} buffer the current descriptor points to and turns it into an \term{Avalon-ST} stream; descriptors live in \hw{vga_prefercher_descriptor_mem}.
    \item \textbf{\hw{vga_dc_fifo}} (dual-clock \term{FIFO}): crosses the stream into the \term{VGA} pixel clock domain.
...
\begin{itemize}
    \item \textbf{\hw{ov7670_ctrl_interface}} (\term{SCCB} master) and \textbf{\hw{ov7670_PCLK_reset_controller}} (camera reset) are the same cores as in \nameref{chap:fpgasteel-i2c-example} (Section~\ref{sec:fpgasteel-i2c-hardware}).
...
Both \term{mSGDMA} cores raise a \term{GIC} interrupt on transfer completion (\hw{f2h_irq0[3]} for the camera, \hw{f2h_irq0[5]} for \term{VGA}), which is what lets \ccode{Camera} and \ccode{Vga} re-arm their descriptors from the \term{ISR} without polling.
...
\ccode{Camera::onTransferComplete()} is the \term{ISR} called every time a frame arrives over the \term{ST}$\rightarrow$\term{MM} stream from the camera; in order, it:
\begin{enumerate}
...
\ccode{Vga} runs the same mechanism on \hw{vga_msgdma} / \hw{vga_prefercher_descriptor_mem}; \ccode{desc0} and \ccode{desc1} each hold a source address only, since this is an \term{MM}--\term{ST} transfer (the destination is the outgoing \term{Avalon-ST} stream, not a memory address).

\ccode{Vga::onTransferComplete()} is the \term{ISR} called every time a \term{MM}$\rightarrow$\term{ST} \term{VGA} frame transfer completes; it checks whether \ccode{vgaPool} has a new frame ready (\ccode{pop()}), then:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \ccode{getDroppedFramesCounter}: returns the \hw{ov7670_data_interface}'s hardware counter, which is incremented whenever the \term{FPGA}-side pipeline itself drops a frame; independent of the software fallback described under \ccode{onTransferComplete()} below.
...
\ccode{Vga} is a singleton that owns two collaborators, by composition, not inheritance: \ccode{PrefetchedMSGDMA}, driving the \hw{vga_msgdma} core; and \ccode{MsgdmaPrefetcherMemory}, for \term{CPU}-side access to that core's on-chip descriptor \term{RAM} (\hw{vga_prefercher_descriptor_mem}). Unlike \ccode{Camera}, it has nothing external to configure over a control bus: \hw{vga_controller} is fixed-function, with no equivalent of \ccode{AvalonSCCB}. Instead, \ccode{Vga} generates its own 8-bar color test pattern internally, so \term{VGA} output is already running the moment \ccode{init()} returns, before any real frame exists. \ccode{desc0}/\ccode{desc1} each hold only a source address, since this is an \term{MM}$\rightarrow$\term{ST} transfer: the destination is the outgoing \term{Avalon-ST} stream, not a \term{DDR} address.

Source: \lstinline[style=inlineFsRepo]!src/Vga.{h,cpp}!.

\begin{itemize}
    \item \ccode{getInstance()}: returns the singleton instance.
    \item \ccode{init(pool)}: requires an \term{RGB24} \ccode{FramePool} sized to the \term{VGA} output resolution (\ccode{FRAME_WIDTH*FRAME_HEIGHT}) (throws \ccode{ExceptionMsgdma} otherwise); generates an internal test pattern at that same resolution (8 vertical color bars: white, red, green, blue, cyan, magenta, yellow, black) and starts two descriptors in ping-pong fashion, both initially reading from the test pattern, so \term{VGA} output is live from \ccode{init()} even before any camera frame is ready.
...
\begin{itemize}
    \item \ccode{convert(src, dst)}, the only public method: converts \ccode{src}, a \ccode{Frame} in any \ccode{Frame::ColorScheme} other than \term{RGB24}, into \ccode{dst}, which must already be \term{RGB24}. Each source format gets its own conversion: \ccode{Mosaic8} is a naive per-pixel demosaic (each pixel keeps only its own \term{Bayer} channel, the other two zeroed; visibly gridded, but adequate to sanity-check capture without a real demosaicing filter), \ccode{YUYV16} gets a full \term{YUV422}$\rightarrow$\term{RGB} conversion, and the packed \term{RGB} formats (\ccode{RGB565}, \ccode{RGB555}, \ccode{RGB444}) are straight bit-unpacked to \term{RGB24}. Throws an exception if \ccode{dst} isn't \term{RGB24}, or if \ccode{src} is a format \ccode{convert} doesn't handle (currently just \term{RGB24} itself: \term{RGB24}$\rightarrow$\term{RGB24} is intentionally not handled here, use \ccode{memcpy} instead).
\end{itemize}

\subsection{HexDisplay class}
\label{subsec:fpgasteel-hex-display}

Singleton driving the custom \hw{hex_display_controller} \term{IP} over a single \term{Avalon-MM} register (bit 31: enabled; bits[19:0]: value 0--999999). Source: \lstinline[style=inlineFsRepo]!src/HexDisplay.{h,cpp}!.
...
\repopath{debugTools/view_frame.py} decodes a raw frame buffer dumped from target memory (e.g.\ retrieved with the \term{Eclipse} \term{IDE}'s debug tools) into a viewable image, for any of the six \ccode{ColorScheme} formats; see its \cmdpath{--help} for options. Useful to check what the camera actually captured, or what \ccode{SimpleFilters::convert()} produced, independently of the \term{VGA} output.
