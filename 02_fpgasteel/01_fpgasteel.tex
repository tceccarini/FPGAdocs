...
\begin{enumerate}
    ...
    \item Write the extracted image to the \term{microSD} card (at least 8 GB) with a disk-imaging tool, e.g. \cmdpath{dd} on \term{Linux} or \term{balenaEtcher} on Windows/Mac.
...
\begin{enumerate}
    \item \cmd{hello_word}: prints ``Hello, World!'' and ``GoodBye'' over the serial console (\term{CPU} caches off).
    \item \cmd{simple_forwarding_example}: captures video from the camera and shows it on \term{VGA} output (camera, adapter, and \term{VGA} monitor required; \term{CPU} caches off).
    \item \cmd{cache_forwarding_example}: same as above, with the \term{CPU}'s data cache enabled (instruction cache off).
    \item \cmd{dualcore_dip_example_no_neon_no_icache}: a small computer-vision pipeline split across both \term{CPU} cores, without \term{NEON} and without the instruction cache (data cache on).
    \item \cmd{dualcore_dip_example_no_neon}: same, without \term{NEON}, with the instruction cache enabled.
    \item \cmd{dualcore_dip_example}: same, using \term{NEON}, with the instruction cache enabled.
\end{enumerate}

\section{Repository layout}

The \prjname{FPGAsteel} repository \cite{FPGASTEEL-REPO}, which is hosted on \term{GitHub}, is laid out as follows:

\begin{itemize}
    \item \repopath{hw/}: \term{FPGA} hardware project (\term{Quartus Prime} + \term{Platform Designer}).
    \begin{itemize}
        \item \repopath{camera_adapter/}: custom \term{PCB} bridging the \term{OV7670} to the \term{DE1-SoC}'s \term{GPIO\_0} header; the board layouts themselves are mirrored in the shared documentation folder (Chapter~\ref{chap:fpgasteel-downloads}).
    ...
    \end{itemize}
    \item \repopath{sw/}: bare-metal example applications (\term{C++}), one folder per example.
    ...
    \item \repopath{repos/}: git submodules (\term{U-Boot}, \term{GHRD}, \term{HPS} hwlib, \term{FPGA} top-level files).
