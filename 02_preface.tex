...
Short code-styled text set in the running prose --- a file path, a command, a register name --- is colored by category throughout this document, so that the kind of thing being named is recognizable at a glance without having to read the surrounding sentence. Table~\ref{tab:preface-inline-styles} lists the eight color categories used, each numbered for quick reference when annotating a proof, plus two further style markers, lettered rather than numbered, that distinguish plain technical terms from ones linked to the Glossary (Appendix~\ref{app:glossary}).
...
\midrule
A & Term & \term{IDE} & a generic technical term, explained inline or assumed known, not linked to the Glossary \\
\lightmidrule
B & Glossary term & \gterm{jtag}{JTAG} & a term whose definition is in the Glossary (Appendix~\ref{app:glossary}); underlined, and footnoted with that definition at its first occurrence in each part \\
\bottomrule
\end{tabularx}
